...
@IonicPage()
@Component({
  selector: 'page-send-reset-code',
  templateUrl: 'send-reset-code.html',
})
export class SendResetCodePage {
  loading: Loading;
  sendRestCodeFormData : FormGroup;
  ...
  constructor(private nav: NavController, private userService: UserServiceProvider, private alertCtrl: AlertController, private loadingCtrl: LoadingController, 
    public formBuilder: FormBuilder,) {
      this.sendRestCodeFormData = this.formBuilder.group({
    ...
    sendResetCode()
    {
      ...
      var userEmail = this.sendRestCodeFormData.controls.email.value.toString();
      var sendRestCodeResponse = this.userService.sendResetCode(userEmail);
